...
import { useAdminEvents, useCancelEvent } from "@/features/admin/hooks/use-admin-hackathons";
...
function HackathonRow({ h }: { h: EventResponse }) {
  const { mutate: cancel, isPending } = useCancelEvent();
  return (
...
export function HackathonManagementPage() {
  const { data: eventsPage, isLoading } = useAdminEvents();
