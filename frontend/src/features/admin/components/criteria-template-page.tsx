...
export function CriteriaTemplatePage() {
  const { data: rawData, isLoading } = useCriteriaTemplates();
  const { mutate: remove } = useDeleteCriteriaTemplate();
  const [showForm, setShowForm] = useState(false);
  // useCriteriaTemplates returns { data: CriteriaResponse[], total: 0 } stub
  const templates = (rawData?.data ?? []) as unknown as CriteriaTemplate[];
