...
export function ScoringPage({ submissionId }: { submissionId: string }) {
  const router = useRouter();
  const { data: submission, isLoading } = useSubmissionScoring(submissionId);
  ...
  const onSubmit = handleSubmit((values) => {
    submitMutation.mutate(
      { submissionId, scores: values.scores },
      {
        onSuccess: () => {
          if (submission?.roundId) {
            router.push(`/judge/rounds/${submission.roundId}`);
          } else {
            router.push("/judge");
          }
        },
  ...
  const onSaveDraft = () => {
    ...
    draftMutation.mutate({ submissionId, scores: current });
  };
