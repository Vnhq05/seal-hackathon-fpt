...
import { exportTeamsCsv } from "@/features/mentor/services/mentor.service";
...
export function MentorTrackHeader({
  ...
  const exportMutation = useMutation({
    mutationFn: () => exportTeamsCsv(trackId),
    onSuccess: (blob) => {
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = "track-data.csv";
      link.click();
  ...
  return (
    <div className="flex items-start justify-between">
      ...
      <button
        ...
        style={{ ...exportBtnStyle, opacity: exportMutation.isPending ? 0.6 : 1 }}
      >
        <DownloadIcon />
        Export Data
      </button>
