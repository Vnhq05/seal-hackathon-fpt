...
export function TrackFormPage({ trackId }: { trackId?: string }) {
  ...
  const { data: existing, isLoading: loadingExisting } = useAdminTrack(trackId ?? "");
  ...
  const {
    ...
    values: isEdit && existing
      ? {
          ...
          mentorId: existing.mentorId ?? undefined,
        }
      : undefined,
  });
  ...
  const onSubmit = (values: TrackFormValues) => {
    if (isEdit && trackId) {
      update({ id: trackId, ...values }, { onSuccess: () => router.push("/admin/tracks") });
    } else {
      create(values, { onSuccess: () => router.push("/admin/tracks") });
    }
  };
